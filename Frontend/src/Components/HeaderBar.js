...
export default function HeaderBar() {
    ...
    return (
        <div style={{...colors, 'textAlign': "center"}}>
            ...
            {(user.username === undefined && <button onClick={() => {userAction({type: 'login'});}}>Log In</button>)}
            {(user.username !== undefined && <button onClick={() => {userAction({type: 'logout'})}}>Log Out</button>)}
        </ div>);
}